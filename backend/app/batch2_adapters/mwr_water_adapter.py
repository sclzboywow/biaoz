...
MWR_BASE = "http://gjkj.mwr.gov.cn"
MWR_LIST_URL = f"{MWR_BASE}/jsjd1/bzh/bzhfbgg/index.htm"


class MwrWaterAdapter(StandardCatalogAdapterMixin):
    ...
    def sync(self, db: Session, source_id: int, options: TrustedSourceSyncOptions) -> TrustedSourceSyncStats:
        ...
        errors = 0
        with make_client(referer=MWR_LIST_URL) as client:
            try:
                html = fetch_html(client, MWR_LIST_URL)
                items = parse_html_list_items(html, MWR_BASE, resource_type=self.config.category_name)
                stats.pages = 1
    ...
    def _search_external_impl(
        ...
        token = keyword.upper()
        for item in items:
            hay = f"{item.standard_no or ''} {item.title}".upper()
            if token not in hay:
                continue
            results.append(
                build_search_result(
...
registry.register(MwrWaterAdapter())
