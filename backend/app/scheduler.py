...
SCHEDULED_CHECK_BATCH_SIZE = 200
...
def check_all_sources_once(storage_root: Path) -> None:
    with SessionLocal() as db:
        ...
        while True:
            source_ids = list(
                db.scalars(
                    select(models.UrlSource.id)
                    .where(
                        models.UrlSource.id > last_id,
                        models.UrlSource.check_frequency != "manual",
                    )
                    .order_by(models.UrlSource.id)
                    .limit(SCHEDULED_CHECK_BATCH_SIZE)
                )
            )
            if not source_ids:
                break
